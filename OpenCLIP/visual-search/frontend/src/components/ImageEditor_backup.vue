...
const isDrawing = ref(false) // 是否正在绘制新的裁剪区域
...
const startCrop = (e) => {
  ...
  if (isCropping.value && !isResizing.value && !isDrawing.value) {
    ...
    if (x >= cropX && x <= cropX + cropW && y >= cropY && y <= cropY + cropH) {
      ...
      // 保存鼠标相对于裁剪框左上角的偏移
      dragOffset.value = { x: x - cropX, y: y - cropY }
      return
    }
  }
...
const updateCrop = (e) => {
  ...
  if (isDragging.value) {
    ...
    // 确保不超出边界
    newX = Math.max(0, Math.min(newX, rect.width - oldWidth))
    newY = Math.max(0, Math.min(newY, rect.height - oldHeight))
    
    // 更新裁剪框位置
    cropStart.value = { x: newX, y: newY }
    cropEnd.value = { x: newX + oldWidth, y: newY + oldHeight }
...
const endCrop = () => {
  if (isDragging.value) {
    isDragging.value = false
    dragOffset.value = { x: 0, y: 0 }  // 清除偏移量
    // 拖动结束后保持裁剪状态
    return
  }
  ...
  // 如果是新绘制的裁剪区域，鼠标松开后停止绘制，但保持裁剪框显示
  if (isDrawing.value) {
    isDrawing.value = false
    // isCropping 保持为 true，让用户可以确认或取消
  }
}
