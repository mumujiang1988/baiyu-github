...
class ERPConfigParser {
  ...
  parseTableColumns() {
    ...
    console.log('🔍 parseTableColumns - 原始 columns:', tableConfig.columns)
    
    const parsedColumns = tableConfig.columns.map(col => {
      console.log('📝 解析列:', col.prop, '-> 类型:', typeof col.prop)
      return {
        ...col,
        ...
        if (key !== 'customers') {
          try {
            ...
            const mappedData = data.map(item => ({
              label: item[value.labelField] || item.label || item.name || item.remark,
              value: item[value.valueField] || item.value || item.dictCode || item.kingdee,
              ...item
            }))
   ...
   * 计算虚拟字段值
   */
  computeVirtualFields(row, virtualFields) {
    if (!virtualFields) return {}

    const computed = {}
    virtualFields.forEach(field => {
      try {
        // 使用 Function 构造函数安全地执行计算规则
        const computeFn = new Function('row', `return ${field.computeRule}`)
        computed[field.name] = computeFn(row)
      } catch (error) {
