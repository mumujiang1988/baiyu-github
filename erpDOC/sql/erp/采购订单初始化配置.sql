-- ============================================
-- ERP 配置 JSON 强制拆分方案 - 采购订单模块导入 SQL（字典重构版）
-- 版本：v4.0 (9 字段拆分 + API 配置版)
-- 日期：2026-04-01
-- 说明：导入采购订单页面配置数据（9 字段强制拆分 + API 配置 + 统一字典 API）
--   - page_config: 页面基础配置
--   - form_config: 表单配置
--   - table_config: 表格列配置
...
-- 级联删除采购订单相关的所有配置
DELETE FROM erp_page_config_history WHERE module_code = 'purchaseorder';
DELETE FROM erp_approval_history WHERE module_code = 'purchaseorder';
DELETE FROM erp_approval_flow WHERE module_code = 'purchaseorder';
DELETE FROM erp_push_relation WHERE source_module = 'purchaseorder' OR target_module = 'purchaseorder';
DELETE FROM erp_page_config WHERE module_code = 'purchaseorder';
...
-- 采购订单页面配置（9 字段强制拆分，包含 api_config、search_config 和 action_config）
INSERT INTO `erp_page_config` (
  ...
  '{
    ...
    "title": "{entityName}管理",
    "permission": "erp:purchaseorder:query",
    "layout": "standard",
  ...
  '{
    ...
    "fields": [
      ...
      },
      {
        ...
        "span": 6,
        "required": true,
        "rules": [{"required": true, "message": "采购日期不能为空", "trigger": "change"}],
  ...
  '{
    "toolbar": [
      {"key": "add", "label": "新增", "icon": "Plus", "permission": "erp:purchaseorder:add", "type": "primary", "position": "left", "handler": "handleAdd"},
      {"key": "edit", "label": "修改", "icon": "Edit", "permission": "erp:purchaseorder:edit", "type": "success", "position": "left", "disabled": "single", "handler": "handleUpdate"},
      {"key": "delete", "label": "删除", "icon": "Delete", "permission": "erp:purchaseorder:remove", "type": "danger", "position": "left", "disabled": "multiple", "handler": "handleDelete", "confirm": "是否确认删除选中的 {count} 条数据？"},
      {"key": "audit", "label": "审核", "icon": "CircleCheck", "permission": "erp:purchaseorder:audit", "type": "success", "position": "left", "disabled": "multiple", "handler": "handleAudit", "confirm": "是否确认审核选中的 {count} 条数据？"},
      {"key": "unAudit", "label": "反审核", "icon": "Close", "permission": "erp:purchaseorder:unAudit", "type": "warning", "position": "left", "disabled": "multiple", "handler": "handleUnAudit", "confirm": "是否确认反审核选中的 {count} 条数据？"},
      {"key": "export", "label": "导出", "icon": "Download", "permission": "erp:purchaseorder:export", "type": "warning", "position": "left", "disabled": "multiple", "handler": "handleExport"},
            ...
            "defaultConditions": [
              {
                ...
                "operator": "eq",
                "value": "${fbill_no}",
                "description": "按单据编号查询明细 (po_order_bill_head.fbill_no = po_order_bill_head_entry.fbillNo)"
              }
            ],
...
-- 配置导入完成
-- ============================================
SELECT '采购订单页面配置导入完成' AS message;
