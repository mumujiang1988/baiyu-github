-- ============================================
-- ERP 配置 JSON 强制拆分方案 - 检验单模块导入 SQL（字典重构版）
-- 版本：v4.0 (9字段拆分 + API配置版)
-- 日期：2026-03-31
-- 说明：导入检验单页面配置数据（9 字段强制拆分 + API配置 + 统一字典 API）
--   - page_config: 页面基础配置
--   - form_config: 表单配置
--   - table_config: 表格列配置
...
-- 级联删除检验单相关的所有配置
DELETE FROM erp_page_config_history WHERE module_code = 'inspection';
DELETE FROM erp_approval_history WHERE module_code = 'inspection';
DELETE FROM erp_approval_flow WHERE module_code = 'inspection';
DELETE FROM erp_push_relation WHERE source_module = 'inspection' OR target_module = 'inspection';
DELETE FROM erp_page_config WHERE module_code = 'inspection';
...
-- 检验单页面配置（9 字段强制拆分，包含 api_config、search_config 和 action_config）
INSERT INTO `erp_page_config` (
  ...
  '{
    "pageId": "inspection",
    "title": "{entityName}管理",
    "permission": "erp:inspection:query",
    "layout": "standard",
  ...
  '{
    ...
    "fields": [
      ...
      },
      {
        ...
        "dictionary": "stock",
        "required": true,
        "props": {"placeholder": "请选择仓库", "clearable": true, "filterable": true}
      },
      {
        "field": "fqc_status",
        "label": "质检状态",
      ...
      {
        "field": "finspect_result",
        "label": "检验结果",
        "component": "input",
        "span": 12,
        "props": {"maxlength": 100, "clearable": true, "placeholder": "请输入检验结果"}
      }
    ]
  }', 
  '{
    ...
    "columns": [
      ...
      {"prop": "fbill_no", "label": "单据编号", "width": 150, "fixed": "left", "align": "left", "visible": true, "resizable": true, "renderType": "text"},
  ...
  '{
    "toolbar": [
      {"key": "add", "label": "新增", "icon": "Plus", "permission": "erp:inspection:add", "type": "primary", "position": "left", "handler": "handleAdd"},
      {"key": "edit", "label": "修改", "icon": "Edit", "permission": "erp:inspection:edit", "type": "success", "position": "left", "disabled": "single", "handler": "handleUpdate"},
      {"key": "delete", "label": "删除", "icon": "Delete", "permission": "erp:inspection:remove", "type": "danger", "position": "left", "disabled": "multiple", "handler": "handleDelete", "confirm": "是否确认删除选中的 {count} 条数据？"},
      {"key": "audit", "label": "审核", "icon": "CircleCheck", "permission": "erp:inspection:audit", "type": "success", "position": "left", "disabled": "multiple", "handler": "handleAudit", "confirm": "是否确认审核选中的 {count} 条数据？"},
      {"key": "unAudit", "label": "反审核", "icon": "Close", "permission": "erp:inspection:unAudit", "type": "warning", "position": "left", "disabled": "multiple", "handler": "handleUnAudit", "confirm": "是否确认反审核选中的 {count} 条数据？"},
      {"key": "export", "label": "导出", "icon": "Download", "permission": "erp:inspection:export", "type": "warning", "position": "left", "disabled": "multiple", "handler": "handleExport"},
  ...
  '{
    ...
    "dictionaries": {
      ...
      "check_type": {
        "type": "api",
        "config": {
          "api": "/erp/engine/dict/union/check_type",
          "useGlobalCache": true,
          "cacheKey": "check_type_dict",
          "cacheTTL": 86400000
        }
      },
      "check_result": {
        "type": "api",
        "config": {
          "api": "/erp/engine/dict/union/check_result",
          "useGlobalCache": true,
          "cacheKey": "check_result_dict",
          "cacheTTL": 86400000
        }
      },
      "bill_status": {
  ...
  '{
    ...
    "entityName": "检验单",
    "entityNameSingular": "检验单",
    "dialogTitle": {"add": "新增{entityName}", "edit": "修改{entityName}"},
    "drawerTitle": "{entityName}详情 - {fbill_no}"
  }',
  '{
    "detail": {
      "enabled": true,
            ...
            "defaultConditions": [
              {
                ...
                "operator": "eq",
                "value": "${fbill_no}",
                "description": "按单据编号查询明细 (receipt_notice_full.fbill_no = receipt_notice_entry.fbill_no)"
              }
            ],
