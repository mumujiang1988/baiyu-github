    ...
    detail: {
      ...
      tabs: [
        //  表格类型页签 (type: "table")
        {
          ...
          icon: "Document",
          type: "table",              //  必须显式指定
          dataField: "entryList",     //  数据存储在 currentDetailRow.entryList
            ...
            defaultConditions: [
              {
                field: "FBillNo",
                operator: "eq",
                value: "${FBillNo}",   //  模板变量，运行时替换为当前单据号
        ...
        //  表单类型页签 (type: "form")  高频错误点
        {
          name: "cost",
          label: "成本暂估",
          icon: "Money",
          type: "form",               //  必须显式指定为 form
          dataField: "costData",      //  数据存储在 currentDetailRow.costData
          ...
          form: {                     //  表单配置在 tab.form 下
            layout: "horizontal",
            labelWidth: 140,
            columns: 3,
            fields: [                 //  字段定义在这里 (不是 tab.fields!)
              {
                prop: "F_hyf",
                label: "海运费 (外币)",
                span: 8,
