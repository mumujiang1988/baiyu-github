...
public class ErpPageConfigController extends BaseController {
    ...
    @GetMapping("/{configId}")
    public ErpResponse<ErpPageConfigVo> getInfo(@NotNull(message = "主键不能为空")
    ...
    @PostMapping()
    public ErpResponse<Void> add(@Validated @RequestBody ErpPageConfigBo bo) {
        return pageConfigService.insertByBo(bo) > 0 ? ErpResponse.ok() : ErpResponse.fail();
    }
    ...
    @PutMapping()
    public ErpResponse<Void> edit(@Validated @RequestBody ErpPageConfigBo bo) {
        return pageConfigService.updateByBo(bo) > 0 ? ErpResponse.ok() : ErpResponse.fail();
    }
    ...
    public ErpResponse<Void> remove(@PathVariable Long[] configIds) {
        return pageConfigService.deleteByIds(configIds) > 0 ? ErpResponse.ok() : ErpResponse.fail();
    }
    ...
    public ErpResponse<ErpPageConfigHistoryVo> getVersionDetail(
            @PathVariable Long configId,
        ...
        return historyVo != null ? ErpResponse.ok(historyVo) : ErpResponse.fail("未找到该版本");
    }
    ...
    public ErpResponse<Void> rollback(@RequestBody Map<String, Object> params) {
        try {
            ...
            return ErpResponse.ok("回滚成功");
        } catch (Exception e) {
            log.error("[ErpPageConfigController] 配置回滚失败", e);
            return ErpResponse.fail("回滚失败：" + e.getMessage());
        }
    }
    ...
    public ErpResponse<Void> upload(@RequestParam MultipartFile file) {
        try {
            ...
            return ErpResponse.fail("导入失败：" + e.getMessage());
        }
    }
    ...
    @PutMapping("/status")
    public ErpResponse<Void> updateStatus(@RequestBody Map<String, Object> params) {
        try {
            Long configId = Long.valueOf(params.get("configId").toString());
            ...
            return ErpResponse.ok("更新成功");
        } catch (Exception e) {
            log.error("[ErpPageConfigController] 更新状态失败", e);
            return ErpResponse.fail("更新失败：" + e.getMessage());
        }
    }
    ...
    public ErpResponse<String> getPageConfig(@PathVariable String moduleCode) {
        ...
        log.info("请求页面配置，moduleCode: {}", moduleCode);
        ...
        log.info("[ErpPageConfigController] 配置预览11：{}", preview);

        // 标准返回：配置数据放在 data 字段，msg 放置返回消息
    ...
    public ErpResponse<String> getDetailConfig(@PathVariable String moduleCode) {
        permissionChecker.checkModulePermission(moduleCode, "query");
        log.info("请求详情配置，moduleCode: {}", moduleCode);
